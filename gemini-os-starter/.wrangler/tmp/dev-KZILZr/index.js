...
// .wrangler/tmp/bundle-n5AsfW/checked-fetch.js
...
// .wrangler/tmp/bundle-n5AsfW/strip-cf-connecting-ip-header.js
...
// node_modules/.pnpm/hono@4.10.2/node_modules/hono/dist/middleware/cors/index.js
...
// node_modules/.pnpm/@upstash+redis@1.35.6/node_modules/@upstash/redis/cloudflare.mjs
...
var RedisKeys = {
  ...
  eventLog: (sessionId) => `events:${sessionId}`,
...
__name(SessionManager, "SessionManager");
var EventLogManager = class {
  ...
  async appendEvents(sessionId, events) {
    ...
    await this.redis.ltrim(key, -1e3, -1);
    await this.redis.expire(key, 7 * 24 * 60 * 60);
  }
  /**
   * Get recent events
   */
  async getRecentEvents(sessionId, count = 20) {
    const key = RedisKeys.eventLog(sessionId);
...
__name(EventLogManager, "EventLogManager");
var AnalyticsManager = class {
  ...
  async getAnalytics(date) {
    ...
    for (const [k, v] of Object.entries(data || {})) {
      analytics[k] = typeof v === "string" ? parseInt(v, 10) : v;
    }
    return analytics;
  }
};
__name(AnalyticsManager, "AnalyticsManager");
...
var RateLimiter = class {
   ...
   */
  async checkLimit(endpoint, ip, config) {
    ...
    const resetIn = Math.ceil((oldestTimestamp + windowMs - now) / 1e3);
    return {
      allowed,
      remaining,
      limit: config.maxRequests,
      resetIn: Math.max(0, resetIn),
      retryAfter: allowed ? void 0 : Math.max(1, resetIn)
    };
  }
  /**
   * Simple counter-based rate limiter (less accurate but faster)
   */
  async checkLimitSimple(endpoint, ip, config) {
    const identifier = config.identifier || ip;
    const key = RedisKeys.rateLimit(identifier, endpoint);
    ...
    return {
      ...
      limit: config.maxRequests,
      resetIn,
      retryAfter: allowed ? void 0 : resetIn
    };
  }
  /**
   * Reset rate limit for an identifier
   */
  async resetLimit(endpoint, ip) {
...
__name(createRateLimitHeaders, "createRateLimitHeaders");
async function rateLimitMiddleware(redis, request, endpoint, config) {
...
// workers/fal-proxy.ts
async function handleFalProxy(c) {
  ...
  try {
    ...
    const queueResponse = await falResponse.json();
...
__name(handleFalProxy, "handleFalProxy");

// workers/session.ts
async function handleSessionAPI(c) {
  const redis = createRedisClient(c.env);
...
var SpriteCacheKV = class {
   ...
   */
  async set(sprite, prompt, type, biome) {
    const key = this.getCacheKey(prompt, type, biome);
   ...
   */
  async delete(prompt, type, biome) {
    const key = this.getCacheKey(prompt, type, biome);
...
__name(SpriteCacheKV, "SpriteCacheKV");
var RoomCacheKV = class {
  constructor(kv) {
    this.kv = kv;
  }
  /**
   ...
   */
  getCacheKey(storySeed, roomId) {
    return `room:${storySeed}:${roomId}`;
  }
  /**
   * Get room from KV cache
   */
  async get(storySeed, roomId) {
...
__name(RoomCacheKV, "RoomCacheKV");
var AudioCacheKV = class {
  constructor(kv) {
    this.kv = kv;
  }
  /**
   ...
   */
  getCacheKey(text, context, emotion) {
   ...
   */
  async get(text, context, emotion) {
   ...
   */
  async set(text, context, url, emotion, ttl = CacheTTL.AUDIO) {
    const key = this.getCacheKey(text, context, emotion);
   ...
   */
  async has(text, context, emotion) {
   ...
   */
  async delete(text, context, emotion) {
    const key = this.getCacheKey(text, context, emotion);
    await this.kv.delete(key);
  }
  /**
...
__name(AudioCacheKV, "AudioCacheKV");

// workers/cache.ts
async function handleCacheAPI(c) {
  const redis = createRedisClient(c.env);
  const rateLimitResponse = await rateLimitMiddleware(
    redis,
    c.req.raw,
    ...
    if (method === "GET") {
      ...
      switch (type) {
        case "sprite": {
          ...
          result = await spriteCache.get(prompt, spriteType, biome || void 0);
          break;
        }
        case "room": {
        ...
        case "audio": {
          ...
          result = await audioCache.get(text, context, emotion || void 0);
          break;
        }
        default:
          return c.json({
    ...
    if (method === "DELETE") {
      ...
      switch (type) {
        case "sprite": {
          const spriteCache = new SpriteCacheKV(c.env.SPRITE_CACHE);
          const [prompt, spriteType, biome] = key.split(":");
          await spriteCache.delete(prompt, spriteType, biome || void 0);
          break;
        }
        case "room": {
          const roomCache = new RoomCacheKV(c.env.ROOM_CACHE);
        ...
        case "audio": {
          const audioCache = new AudioCacheKV(c.env.AUDIO_CACHE);
          const [context, text, emotion] = key.split(":");
          await audioCache.delete(text, context, emotion || void 0);
          break;
        }
        default:
          return c.json({
...
__name(handleCacheAPI, "handleCacheAPI");

// workers/analytics.ts
async function handleAnalyticsAPI(c) {
  const redis = createRedisClient(c.env);
  const rateLimitResponse = await rateLimitMiddleware(
    redis,
    c.req.raw,
...
__name(handleAnalyticsAPI, "handleAnalyticsAPI");
...
// .wrangler/tmp/bundle-n5AsfW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-n5AsfW/middleware-loader.entry.ts
